...
export const getCandidates = async (req, res) => {
  ...
  try {
    //  FIX: Kiểm tra req.user tồn tại trước
    if (!req.user) {
      return res.json(toResultError({ statusCode: 401, msg: "User not authenticated" }));
    }
    ...
    if (jobId) {
      ...
      const allCandidates = await Candidate.find().populate(
        "user_id",
